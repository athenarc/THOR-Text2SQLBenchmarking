...
public class IREngine {
    ...
    // Returns a list that associates every table in the database with its tuple set.
    ...
    private TupleSet generateTupleSetOfTable(String query, SQLTable table,
        ...
        for (SQLColumn column : table.getColumns()) {
            ...
            if (column.getType().isTextual() && column.isIndexed()) {
                ...
                // String againstStmt = this.prepareKeywordsForOrBooleanSearch(this.keywords);
    ...
    /**
     * Prepare the list of keywords for an OR boolean search in MySQL FULLTEXT
     * indexes. Put every keyword with 2 or more words in parenthesis and add
     * "+" symbols in each of the words so results of this query will contain both 
     * or none. For all keywords with less than 2 words just put a space in front of them.
     * 
     * @param keywords
     * @return A string ready to by placed in the against(?) part of a FULLTEXT search query.
     */
    public String prepareKeywordsForOrBooleanSearch(List<String> keywords) {
        String orStmt = new String();
        for (String keyword: keywords) {
            orStmt += " " + ( 
                keyword.contains(" ") ? 
                "(" + DatabaseUtil.prepareForAndBooleanSearch(keyword) +")" : keyword 
            );
        }
        return orStmt;
    }
