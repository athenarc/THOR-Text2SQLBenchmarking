...
public class OverloadedTuple extends SQLTuple implements ResultInterface {
    ...
    /**
     * @return True if tuple does not contain any keyword.
     */
    public boolean containsNoKeyword() {
        int countNoContainedKeywords = 0;
        for (Map.Entry<String,Integer> entry: this.keywordFrequencies.entrySet()) {
            if (entry.getValue() == 0){
                countNoContainedKeywords++;
            }
        }

        if (countNoContainedKeywords == this.keywordFrequencies.size())
            return true;
        else 
            return false;
    }
    ...
    // Get Tuple 's Signature.
    public Signature getSignature() {
        // Create and fill the signature object.
        Signature signature = new Signature();
        signature.fill(keywordFrequencies);
    ...
    public double computeScore(Block block) {
        return this.computeScoreA(block) * block.computeScoreB() * block.getSizeNormalizationFactor();
    }

    // Computes the score_a value of a tuple (which is the result of a join).
    private double computeScoreA(Block block) {
        double sum = 0.0;
        double s = 0.2; // Constant defined in the paper.
        Signature blockSignature = block.getSignature();

        // Loop through the keywords contained in the block, since its results contain those exact keywords.
        for (String keyword : blockSignature.getContainedKeywords()) {
            double a = 1 + Math.log(1 + Math.log(blockSignature.getKeywordFrequency(keyword)));
            double b = this.getLengthOfColumnThatContainsKeyword(keyword) / block.getNetworkColumnsAverageLength();
            double c = a / ((1.0 - s) + s * b);
            sum += c * Math.log(block.getKeywordIdf(keyword));
        }

        return sum;
    }

    // Returns the length (in words) of the attribute that contains the given keyword.
    public int getLengthOfColumnThatContainsKeyword(String keyword) {
        for (SQLValue value : this.values) {
            // Check that the column is indexed.
            if ((this.attributes.get(this.values.indexOf(value)).isIndexed()) && (value.contains(keyword))) {
    ...
    public void fill(List<SQLColumn> columns, ResultSet rs) {
        try {
            ...
            for (int index = 0; index < metadata.getColumnCount(); index++) {
                ...
                if (column == null && metadata.getColumnName(index + 1).equals("score"))
    ...
    public String toString() {
        ...
        for (int index = 0; index < attributesNumber; index++) {
            rowOfValues += "|" + PrintingUtils.addStringWithLeadingChars(
                ...
                this.getValues().get(index).toString().replace("\n", " "),
                " "
            );
        }
        rowOfValues += "|";
