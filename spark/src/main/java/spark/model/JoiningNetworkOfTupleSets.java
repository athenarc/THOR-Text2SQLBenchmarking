...
public class JoiningNetworkOfTupleSets {
    ...
    // Returns the sum of the average lenghts (in words) of the columns of all tables in the network.
    public double getNetworkColumnsAverageLength() {
        double sum = 0.0;
        
        for (TupleSet tupleSet : this.getNonFreeTupleSets()) {
            for (SQLColumn column : tupleSet.getTable().getIndexedColumns()) {
                sum += column.getAverageLength();
            }
        }
    ...
    // the map with the keyword idf values, and the sumidf value.
    ...
    // Returns the idf of a specific keyword.
    // Returns a default value of zero (0) if the keyword was not found in the map.
    public Double getKeywordIdf(String keyword) {
        return this.keywordIdfs.getOrDefault(keyword, 0.0);
    }

    // Returns the maximum idf value from the keywordIdfs map.
    public Double getMaximumKeywordIdf() {
        Double max = -1.0;
    ...
    // The sumidf is the sum of the idf values of every keyword contained in the network.
    ...
    public String debugPrint() {
        ...
        while (!queue.isEmpty()) {
            ...
            str += PrintingUtils.addPrefixCharNumTimesToStr(depth, node.getJoinableExpressionsAbbreviation() + ": [", "  ");
            ...
            OverloadedTupleList tuples = new OverloadedTupleList(node.getTupleSet().getTuples());            
            str += "Node's Tuples\n" + tuples.toString(true);
