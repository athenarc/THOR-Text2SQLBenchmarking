...
public class OverloadedTupleList {
    ...
    public void print(Boolean printScore) {
        ...
        System.out.println("SQL Query: " + this.tuples.get(0).getQuery());
        ...
        for (OverloadedTuple tuple: this.tuples) {
            ...
            for (int index = 0; index < attributesNumber; index++) {
                rowOfValues += "|" + PrintingUtils.addStringWithLeadingChars( maxColumnLengthOfStrings[index],
                                        PrintingUtils.escapeCharacter(tuple.getValues().get(index).toString(), '\n', ' '),
                                        " ");
            }
            rowOfValues += "|";
    ...
    public String toString(Boolean printScore) {
        String str = new String();
        if (this.tuples == null || this.tuples.isEmpty()) return null;
        ...
        str += tablesInString.substring(0, tablesInString.length()-5) + "\n";
